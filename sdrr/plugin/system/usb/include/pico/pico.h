...
// A replacement pico.h header to avoid needing the Pico SDK to build
// picobootx
...
// RP2040 TRM 2.1.2: each peripheral register block has three aliases:
//   +0x1000  XOR
...
//--------------------------------------------------------------------+
// Busy wait
// The USB SIE_CTRL / BUF_CTRL concurrent access requirement (TRM
// 4.1.2.5.1) needs at least 12 cycles @ 48 MHz USB clock.
...
//--------------------------------------------------------------------+
// DPRAM structures - device mode (TRM 4.1.3)
//
// Layout:
...
//--------------------------------------------------------------------+
// DPRAM structures - host mode (TRM 4.1.3, host view)
//--------------------------------------------------------------------+
typedef struct {
    io_rw_32 ctrl;
...
//--------------------------------------------------------------------+
// USB hardware register block (TRM 4.1.4)
//
//   0x000  ADDR_ENDP
//   0x004  ADDR_ENDP1  )
...
//--------------------------------------------------------------------+
// USB register bit definitions (TRM 4.1.4)
//--------------------------------------------------------------------+
...
//--------------------------------------------------------------------+
// IRQ - prototypes only; implementations are provided by the
// application (irq_add_shared_handler, irq_remove_handler,
// irq_set_enabled)
//--------------------------------------------------------------------+
...
void irq_add_shared_handler(uint irq, irq_handler_t handler, uint8_t order_priority);
void irq_remove_handler(uint irq, irq_handler_t handler);
