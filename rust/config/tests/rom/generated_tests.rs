...
fn test_rom_2316_specs() {
    ...
    assert!(control.iter().all(|c| c.line_type == ControlLineType::Configurable));
}
...
fn test_rom_27128_specs() {
    ...
    assert!(control.iter().any(|c| c.name == "ce" && c.line_type == ControlLineType::FixedActiveLow));
    assert!(control.iter().any(|c| c.name == "oe" && c.line_type == ControlLineType::FixedActiveLow));
...
fn test_all_rom_types_parse() {
    ...
    for type_name in types {
        assert!(RomType::try_from_str(type_name).is_some(), "Failed to parse {}", type_name);
    }
}