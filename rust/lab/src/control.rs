...
impl Control {
    ...
    pub fn new(rom: Rom) -> Self {
        ...
        let rpc = unsafe 
        {
            ...
            Target::new(cmd_memory, rsp_memory)
                .expect("Failed to create RPC Channels")
        };
    ...
    pub async fn run(&mut self) -> ! {
        loop {
            ...
            let cmd_size = loop {
                match self.rpc.command_size().expect("RPC Channel error getting command size") {
                    Some(size) => if size >= core::mem::size_of::<Command>() {
                        break size
                    } else {
            ...
            let received = self.rpc.read_command(&mut cmd_data).expect("RPC Channel error reading command");
            ...
            // sent).
            self.rpc.mark_command_processed().expect("RPC Channel error marking command processed");
        }
    ...
    async fn handle_command(&mut self, command: Command, cmd_data: &[u8]) {
        ...
        Command::Unknown => {
            // Ignore
            warn!("Received unknown command 0x{:02x}{:02x}{:02x}{:02x}, ignoring", cmd_data[0], cmd_data[1], cmd_data[2], cmd_data[3]);
        }
    }
    ...
    fn send_response_no_data(&mut self, response: Response) {
        ...
        self.rpc.send_response(&buf).expect("RPC Channel error sending response (no data)");
    }

    fn send_response_data(&mut self, data: &[u8]) {
        self.rpc.send_response(data).expect("RPC Channel error sending response (with data)");
    }
}